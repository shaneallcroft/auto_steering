...
void SubAndPub::cmdVelCallback(const sensor_msgs::LaserScan::ConstPtr & scan_msg)
{
  ...
  if (LIDAR_IS_ON_THE_CORRECT_WAY) {
    for (int i = scan_msg->ranges.size() / CONE_DENOMINATOR;
      i < (scan_msg->ranges.size() - (scan_msg->ranges.size() / CONE_DENOMINATOR)); ++i)
    {
    ...
    for (int i = 0; i < scan_msg->ranges.size(); ++i) {
      if (i > (scan_msg->ranges.size() / CONE_DENOMINATOR) / 2 &&
        i < (scan_msg->ranges.size() - ((scan_msg->ranges.size() / CONE_DENOMINATOR) / 2)))
      {
        isRightSide = false;
        continue;
      }
